...
class CapabilityDetector {
  ...
  static get _cache() { return this.#cache; }
  static set _cache(value) { this.#cache = value; }
  ...
  static detect() {
    ...
    try {
      ...
      secureDevLog('warn', 'CapabilityDetector', 'Composite detection failed', { error });
    }
    ...
    if (CONFIG.DEBUG) {
      secureDevLog('info', 'CapabilityDetector', 'Capabilities detected', capabilities);
    }
  ...
  static #testComposite() {
    ...
    try {
      ...
      const animation = testEl.animate(
        [{ opacity: 0 }, { opacity: 1 }],
        { duration: 1, composite: 'replace' }
      );
      
      const hasComposite = animation && 
...
class ElementValidator {
  ...
  static validateElement(element, expectedId = null) {
    ...
    if (expectedId && element.id !== expectedId) {
      throw new Error(`Element ID mismatch: expected ${expectedId}, got ${element.id}`);
    }
  ...
  static queryElementSafely(selector, context = document) {
    ...
    try {
      ...
      secureDevLog('error', 'ElementValidator', 'Query failed', { selector, error });
      return null;
    }
  }
}
...
class EnhancedHeaderAnimator {
  ...
  constructor(headerElement) {
    ...
    try {
      this.#header = ElementValidator.validateElement(headerElement, CONFIG.HEADER_ID);
      ...
      if (CONFIG.DEBUG) {
        ...
        secureDevLog('info', 'EnhancedHeaderAnimator', 'Initialization complete', {
  ...
  #setupElements() {
    ...
    if (elementCount > CONFIG.SECURITY.MAX_ELEMENTS) {
      throw new Error(`Too many elements: ${elementCount} > ${CONFIG.SECURITY.MAX_ELEMENTS}`);
    }
    
    for (const [key, selector] of Object.entries(CONFIG.ELEMENT_SELECTORS)) {
      const element = ElementValidator.queryElementSafely(selector, this.#header);
      
      if (element) {
        this.#elements.set(key, element);
        if (CONFIG.DEBUG) {
          secureDevLog('debug', 'EnhancedHeaderAnimator', `Element found: ${key}`);
        }
      } else if (CONFIG.DEBUG) {
        secureDevLog('warn', 'EnhancedHeaderAnimator', `Element not found: ${key} (${selector})`);
      }
    }
  }
  
  #setupViewTransitions() {
    if (this.#capabilities.viewTransitions) {
      this.#header.style.viewTransitionName = 'main-header';
      
      if (CONFIG.DEBUG) {
        secureDevLog('info', 'EnhancedHeaderAnimator', 'View Transitions enabled');
      }
    }
  }
  ...
  #createAnimations() {
    ...
    try {
      ...
      const headerAnimation = this.#header.animate(headerKeyframes, baseOptions);
    ...
    const animationConfigs = [
      {
        key: 'navbarContainer',
        keyframes: [
          { paddingTop: '1rem', paddingBottom: '1rem', transform: 'translateZ(0)' },
          { paddingTop: '0.25rem', paddingBottom: '0.25rem', transform: 'translateZ(0)' }
        ]
      },
      {
        key: 'headerLogo',
    ...
    for (const { key, keyframes } of animationConfigs) {
      ...
      if (element) {
        try {
          ...
          secureDevLog('error', 'EnhancedHeaderAnimator', `Failed to create animation for ${key}`, { error });
        }
      }
    }
  }
  ...
  #updateAnimationState() {
    ...
    try {
      ...
      if (CONFIG.DEBUG && animationsUpdated > 0) {
        secureDevLog('debug', 'EnhancedHeaderAnimator', 'Animation state updated', {
  ...
  #handleError(message, error) {
    ...
    if (this.#performanceMetrics.errorCount > CONFIG.PERFORMANCE.MAX_ANIMATION_RETRIES) {
      this.destroy();
    }
  }
  ...
  getMetrics() {
    return {
      ...
      capabilities: this.#capabilities.level
    };
  }
  ...
  destroy() {
    ...
    try {
      ...
      if (CONFIG.DEBUG) {
        secureDevLog('info', 'EnhancedHeaderAnimator', 'Destroyed successfully', this.getMetrics());
      }
      
    } catch (error) {
      secureDevLog('error', 'EnhancedHeaderAnimator', 'Destruction failed', { error });
    }
  }
}
...
class HeaderAnimationManager {
  ...
  static initialize() {
    try {
      const header = ElementValidator.queryElementSafely(`#${CONFIG.HEADER_ID}`);
      
      if (!header) {
        if (CONFIG.DEBUG) {
          secureDevLog('warn', 'HeaderAnimationManager', `Header not found: #${CONFIG.HEADER_ID}`);
        }
        this.#instance = null;
        return;
      }
      ...
      if (CONFIG.DEBUG) {
        secureDevLog('info', 'HeaderAnimationManager', 'Initialization successful');
      }
      
    } catch (error) {
      secureDevLog('error', 'HeaderAnimationManager', 'Initialization failed', { error });
      this.#instance = null;
...
export { EnhancedHeaderAnimator, HeaderAnimationManager, CapabilityDetector, ElementValidator };
